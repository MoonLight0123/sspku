...
typedef struct short_name_entry {//32byte
...
typedef struct long_name_entry {//32byte
    uint8       order;
    wchar       name1[5];//wchar占两个字节
    uint8       attr;
    uint8       _type;
    uint8       checksum;
    wchar       name2[6];
...
static struct {
    ...
    struct {
        ...
        uint32  root_clus;
    } bpb;//BPB（BIOS Parameter Block），即 BIOS 参数块
...
int fat32_init()
{
    ...
    for(struct dirent *de = ecache.entries; de < ecache.entries + ENTRY_CACHE_NUM; de++) {
        ...
        de->dirty = 0;
        de->parent = 0;
        de->next = root.next;//将所有空闲的dirent cache挂到root上
        de->prev = &root;
...
struct dirent *lookup_fromEPandPath(char *path, int parent, char *name,struct dirent* entry)
{
    struct dirent  *next;
    // if (*path == '/') {
    //     entry = edup(&root);
