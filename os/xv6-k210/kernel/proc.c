...
proc_pagetable(struct proc *p)
{
  ...
  if(mappages(pagetable, TRAMPOLINE, PGSIZE,
              (uint64)trampoline, PTE_R | PTE_X) < 0){//trapoline在物理地址的kernel text部分
    uvmfree(pagetable, 0);
    return NULL;
  }
  ...
  if(mappages(pagetable, TRAPFRAME, PGSIZE,
              (uint64)(p->trapframe), PTE_R | PTE_W) < 0){//p->trapframe在物理地址的etext到phystop的某一页中
...
// uchar initcode[] = {
//   0x17, 0x05, 0x00, 0x00, // auipc   a0,0x0
//   0x03, 0x35, 0x05, 0x00, // ld      a0,0(a0)
//   0x97, 0x05, 0x00, 0x00, // auipc   a1,0x0
//   0x83, 0xb5, 0x05, 0x00, // ld      a1,0(a1)
//   0x93, 0x08, 0x70, 0x00, // li      a7,7
//   0x73, 0x00, 0x00, 0x00, // ecall

//   0x93, 0x08, 0xd0, 0x05, // li      a7,93
//   0x73, 0x00, 0x00, 0x00, // ecall
//   0xef, 0xf0, 0x9f, 0xff, // jal     18 <exit>

//   0x2f, 0x69, 0x6e, 0x69, // .word   0x696e692f
//   0x74, 0x00, 0x00, 0x24, 
//   0x00, 0x00, 0x00, 0x00, // Padding to align data (if needed)
//   0x00, 0x00, 0x00, 0x00  // More padding (if needed)
// };
...
exit(int status)
{
  ...
  // p->xstate = status;
  p->xstate = status<<8;//为了通过waitpid这个测试样例？
  // printf("pid:%d,p->state:%d\n",p->pid,p->xstate);
  p->state = ZOMBIE;
...
int
wait4(int pid,uint64 addr,int option)
{
  struct proc *np;
  int havekids, kid_pid;
  struct proc *p = myproc();
  ...
  for(;;){
    ...
    for(np = proc; np < &proc[NPROC]; np++){
      ...
      // acquiring the lock first would cause a deadlock,
      // since np might be an ancestor, and we already hold p->lock.
      if(p==np)continue;//防止重复对p上锁
      if(np->parent == p || pid==np->pid || pid==-1){//pid=-1时可以等待任何子进程
        // np->parent can't change between the check and the acquire()
        // because only the parent changes it, and we're the parent.
        acquire(&np->lock);
        if(np->parent == p)
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        sfence_vma();
        // printf("[scheduler]end proc with pid: %d\n", p->pid);
...
int forkAndExec(uint64 fnAddr,uint64 spAddr)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  // Cause fork to return 0 in the child.
  np->trapframe->a0 = 0;
  np->trapframe->a1 = fnAddr;
  np->trapframe->sp = spAddr;
  // increment reference counts on open file descriptors.
  for(i = 0; i < NOFILE; i++)
    if(p->ofile[i])
      np->ofile[i] = filedup(p->ofile[i]);
